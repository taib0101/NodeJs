...
handle.requestResponseHandler = (request, response) => {
    ...
    const currentRouteHandler =
        !importedRoute["sampleRoute"].includes(trimmedPathName) ?
            !importedRoute["userRoute"].includes(trimmedPathName) ?
                !importedRoute["tokenRoute"].includes(trimmedPathName) ?
                    !importedRoute["checkRoute"].includes(trimmedPathName) ?
                        importedRoute.noRouteHandler : importedRoute.checkHandler
                    : importedRoute.tokenHandler
